...
export const StationMap = ({ stations, loading = false, height = '500px', onStationClick }) => {
    ...
    // Normalise les données pour éviter les undefined et aligner les champs attendus
    ...
    }, [normalizedStations, searchTerm, statusFilter]);

    const statusCounts = useMemo(() => {
        if (!normalizedStations.length) return { all: 0, available: 0, low: 0, empty: 0, full: 0 };
        return {
            all: normalizedStations.length,
            available: normalizedStations.filter(s => s.status === 'available').length,
            low: normalizedStations.filter(s => s.status === 'low').length,
            empty: normalizedStations.filter(s => s.status === 'empty').length,
            full: normalizedStations.filter(s => s.status === 'full').length
        };
    }, [normalizedStations]);
